...
namespace Ventas.Web.Http
{
    public class HttpRepository : IHttpRepository
    {
        ...
        public ObjIn? Get<ObjIn>(string url, ObjIn? response) where ObjIn : BaseReponse
        {
        ...
        public ObjOut? Set<ObjIn, ObjOut>(string url, ObjIn request, ObjOut? response) where ObjOut : BaseReponse
        {
            using (var httpClient = this.httpClientFactory.CreateClient())
            {
